...
// Initialize - Fetch APK info from localStorage (GitHub-friendly)
...
// Fetch APK information from localStorage (GitHub-friendly)
async function fetchApkInfo() {
  try {
    console.log('📡 Fetching APK info from localStorage...');
    
    // First check for direct config
    let configData = localStorage.getItem('apk_config');
    
    // If no direct config, check if we have APKs array and create config from latest
    if (!configData) {
      console.log('⚠️ No direct config found, checking APKs array...');
      const apks = JSON.parse(localStorage.getItem('apks') || '[]');
      
      if (apks.length > 0) {
        console.log('🔧 Found APKs array, creating config from latest APK...');
        const latestApk = apks[apks.length - 1]; // Get most recent
        
        // Create config from latest APK
        const generatedConfig = {
          latest_version: latestApk.version,
          latest_version_code: latestApk.versionCode,
          file_path: latestApk.fileName,
          file_size: latestApk.fileSize,
          download_url: latestApk.downloadUrl,
          last_updated: latestApk.uploadDate,
          release_notes: latestApk.releaseNotes || 'No release notes'
        };
        
        // Save the generated config
        localStorage.setItem('apk_config', JSON.stringify(generatedConfig));
        configData = JSON.stringify(generatedConfig);
        console.log('✅ Generated config from APKs array');
      }
    }
    ...
    if (configData) {
      ...
      apkInfo = {
        ...
        version_code: data.latest_version_code,
      ...
      console.log('📦 Full APK Info:', apkInfo);
    } else {
      console.warn('⚠️ No APK config found in localStorage');
      
      // Provide default values
      apkInfo = {
        filename: 'mParivahan.apk',
        version: '1.0.0',
        version_code: 1,
...
// Download APK from GitHub-friendly system
async function downloadAPK(event) {
  ...
  try {
    ...
    // Get the download URL directly from our config
    const downloadUrl = apkInfo.download_url || './apk_files/mParivahan.apk';
    const filename = apkInfo.filename || 'mParivahan.apk';
    
    console.log('🔗 Download URL:', downloadUrl);
    console.log('📱 Filename:', filename);

    // For mobile browsers, use window.location for better compatibility
    ...
    showNotification('Download Started', `${filename} is downloading...`, 'success');
